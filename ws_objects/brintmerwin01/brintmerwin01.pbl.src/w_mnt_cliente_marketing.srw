$PBExportHeader$w_mnt_cliente_marketing.srw
...
if tab_1.tp_1.dw_1.uf_ha_cambiado() or &
   ...
   tab_1.tp_5.dw_5.uf_ha_cambiado() or &
	...
	li_resp = MessageBox(gsNomApl,'¿Desea grabar los cambios?',question!, YesNoCancel!, 1)
	If li_resp = 1 Then
...
string text = "Ultima Actualización"
end type
...
// Validación de Duplicidad de Doc. Id. - mah 09/10/2003
...
// Validación de longitud de Doc. Id. - mah 30/09/2003
...
if gsPais='PER' then
	if ln_verifica_ldoc <> 1 then
		ls_mensaje += 'La longitud del doc. de identidad no es válido.~n'
		lb_cambio = false
	end if
	ln_verifica = io_cliente.f_int_ntdide(ls_tipdide, ls_ntdide, this.getitemstring(row,"ntdide_comp"))
	if ln_verifica = 0 then
		ls_mensaje += 'Nro. RUC no es válido.'
		lb_cambio = false
	end if
	// Se quita validación de doc. identidad. mgg 18/08/2004
	...
	if isnull(ll_motivoestado) and li_idestado = 2 then
		MessageBox ("Warning","Poner el Motivo de la Inactivación")
		lb_cambio = false
	end if
end if                
...
if dwo.name='b_grupoecon' then
	if ii_tipo = 2 then		
		lst_par_in.titulo = 'Busqueda de Grupo Económico'	
...
if dwo.name='b_idae' then
	if ii_tipo = 2 then			
		lst_par_in.titulo = 'Busqueda de Actividad Económica'		
...
integer height = 1768
long backcolor = 79741120
string text = "Medios de Comunicación"
long tabtextcolor = 33554432
long tabbackcolor = 79741120
...
/*if isnull(this.getitemstring(row,'numcomcli')) or this.getitemstring(row,'numcomcli')="" then
	messagebox('Contactos de Empresa', 'Debe ingresar el campo Número')
	return false
end if*/
...
if isnull(this.getitemstring(row,"idmedcom")) or this.getitemstring(row,"idmedcom")= "" then
	messagebox('Medios de Comunicación','No ha seleccionado un medio de Comunicación.')
	return false
end if
//if not isnumber(this.getitemstring(row,"ddd")) and not isnull(this.getitemstring(row,"ddd")) and this.getitemstring(row,"ddd")<>'' then
//	messagebox('DDD','El campo DDD debe ser llenado con números.')
//	return false
//end if
//if isnull(this.getitemstring(row,"numcomcli")) or this.getitemstring(row,"numcomcli")= "" then
//	messagebox('Medios de Comunicación','El campo Med. Comunicación es obligatorio.')
//	return false
//end if
...
//	messagebox('Anexo','El campo Anexo debe ser llenado con números.')
//	return false
//end if
return true
end event
...
integer height = 1768
long backcolor = 79741120
string text = "Información x Servicio"
long tabtextcolor = 33554432
long tabbackcolor = 79741120
...
integer height = 1272
long backcolor = 79741120
string text = "Detalle de Facturación"
long tabtextcolor = 33554432
long tabbackcolor = 79741120
...
integer height = 1768
long backcolor = 79741120
string text = "Estructura de Atención"
long tabtextcolor = 33554432
long tabbackcolor = 79741120
...
choose case as_columna
	case 'codect'
	if ii_permiso_est_aten = 1 then	
		messagebox("Aviso", "Opción no valida")		
	else
	if ii_tipo = 2 or ii_tipo = 3 then		
		lst_par_in.titulo = 'Busqueda de Ejecutivo de Cuenta'	
		openwithparm(w_bus_ejecutivo, lst_par_in)
...
event ue_guardar;call super::ue_guardar;//Modificación de usuario que se haga cambios en esta seccion de estructura de atencion
