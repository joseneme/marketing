$PBExportHeader$w_mnt_tabla_tipo.srw
forward
	...
	CHOOSE CASE dw_maestro.dataobject
		CASE "dw_categoria"
			if isnull(dw_maestro.getitemstring(row,'descripcion')) then
				Messagebox('Categoría de Cliente', 'La descripción de la categoria no puede~r~n' + &
								'ser vacía.~r~nError en la fila: ' + string(row) )
				return false
			end if
		...
		CASE "dw_gr_calificacion_visual"
			if isnull(dw_maestro.getitemstring(row,'calificacion')) or &
				...
				Messagebox('Calificación Visual', 'La Calificación Visual no puede~r~n' + &
								'ser vacía o espacios en blanco.~r~nError en la fila: ' + string(row) )
				return false
			end if
		CASE "dw_gr_estado_calificacion"
			if isnull(dw_maestro.getitemstring(row,'calificacion')) or &
				trim(dw_maestro.getitemstring(row,'calificacion'))='' then
				Messagebox('Estatus Calificación', 'El Estatus Calificación no puede~r~n' + &
								'ser vacía o espacios en blanco.~r~nError en la fila: ' + string(row) )
				return false
			end if
		...
		CASE "dw_grupoeconomico"
			if isnull(dw_maestro.getitemstring(row,'nombre')) or &
				...
				Messagebox('Grupo Económico', 'El Grupo Económico no puede~r~n' + &
		...
		CASE "dw_gr_medio_comunicacion"
			if isnull(dw_maestro.getitemstring(row,'medcom')) or &
				...
				Messagebox('Medio de Comunicación', 'El Medio de Comunicación no puede~r~n' + &
							'ser vacío o espacios en blanco.~r~nError en la fila: ' + string(row) )
				return false
			end if
		...
		CASE "dw_gr_estado_activacion"
			if isnull(dw_maestro.getitemstring(row,'descripcion')) or &
				trim(dw_maestro.getitemstring(row,'descripcion'))='' then
				Messagebox('Estatus de Activación', 'El Estatus de Activación no puede~r~n' + &
							'ser vacío o espacios en blanco.~r~nError en la fila: ' + string(row) )
				return false
			end if
