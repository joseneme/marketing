$PBExportHeader$w_mnt_solution_tree.srw
...
if ll_count > 0  then
	messagebox("Anormalidades","No se puede activar porque el nodo padre no está activo.")
	return 
end if
select count(*) into :ll_count
...
if ll_count>0 then
  ...
  where id_solution= :ll_id;
  commit;
  messagebox("Anormalidades","Se activó el nodo.")
end if
...
if ll_count>0 then
  ...
  where id_solution= :ll_id;
  commit;
  messagebox("Anormalidades","Se desactivó el nodo.")
end if
...
ii_niveles = li_levelsolution//Número de niveles del TreeView
