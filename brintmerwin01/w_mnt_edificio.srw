HA$PBExportHeader$w_mnt_edificio.srw
...
if f_dw_modify(dw_maestro) then
	if isnull(dw_maestro.getitemnumber(ll_row_maestro, "inmueble")) then
		messageBox ("Error al grabar los datos","campo 'Inmueble' vac$$HEX1$$ed00$$ENDHEX$$o")
		return false
	end if
	if isnull(dw_maestro.getitemstring(ll_row_maestro, "codubi")) then
		messageBox ("Error al grabar los datos","campo 'Distrito' vac$$HEX1$$ed00$$ENDHEX$$o")
		return false
	end if
...
//		messageBox ("Error al grabar los datos","El telefono del Administrador solo acepta n$$HEX1$$fa00$$ENDHEX$$meros.")
//		return false
//	end if
	if not isnull(dw_maestro.getitemnumber(ll_row_maestro, "codigo")) then
		...
		if ll_asignado>0 then
			messageBox ("Error al grabar los datos","La direcci$$HEX1$$f300$$ENDHEX$$n indicada ya pertenece a otro edificio.")
			return false
		end if
	end if
...
for i=1 to ll_filas - 1
	for j=i+1 to ll_filas
		if getitemnumber(i,'idinmueble')=getitemnumber(j,'idinmueble') then
			messagebox('Direcciones Alternativas','No puede asignar la misma Direcci$$HEX1$$f300$$ENDHEX$$n mas de 1 vez.')
			return false
		end if
	next
next
